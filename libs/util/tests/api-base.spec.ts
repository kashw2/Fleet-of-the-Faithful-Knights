...
describe('ApiBase', () => {
  test('should remove endpoint / from base with ending /', t => {
    ...
    t.expect(result).toEqual('https://www.example.com/endpoint');
  });
  ...
  test('should remove whitespace in url', t => {
    ...
    t.expect(result).toEqual('http://localhost:3000/endpoint');
  });
  ...
  test('should fail when a future value is returned', async t => {
     ...
     * `Either<string, any>`.
     */
    t.expect(result.isLeft()).toEqual(true);
  });
});
